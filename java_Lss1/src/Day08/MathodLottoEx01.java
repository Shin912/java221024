...
public class MathodLottoEx01 {
	...
	public static void main(String[] args) {
		...
		System.out.println("일치하는 로또 번호 갯수: " + countLotto(lotto,user));
		int rank = getRank(lotto,user,bonus);
		printRank(rank);
		
	}
	
	/* 기능 : 주어진 두 배열에서 일치하는 숫자의 갯수를 알려주는 메소드
	 * 매개변수 : 배열1, 배열2 => []lotto, []user
	 * 리턴타입 : 일치하는 숫자의 갯수 => 정수 => int 
	 * 메소드명 : countLotto
	 * */
	...
	/* 기능 : 로또 번호(보너스번호 포함)와 입력번호가 주어지면 등수를 알려주는 메소드
	 * 매개변수 : 로또번호, 입력번호 => []lotto, []user, bonus
	 * 리턴타입 : 등수 => 정수 => int
	 * 메소드명 : getRank
	 * */
	public static int getRank(int []lotto, int []user, int bonus) {
		if(lotto == null || user == null) {
			return 0;
		}
		int sameCount = countLotto(lotto,user);
