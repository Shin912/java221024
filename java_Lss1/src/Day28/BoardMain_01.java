...
public class BoardMain_01 {
	...
	private static void runMenu(int menu) {
		...
		case 4:
			...
			break;
		default:
			throw new RuntimeException("잘못된 메뉴 입력!");
		}
		
	}
	private static void categoryMenu() {
		//관리자 확인=>관리자가 아니면 메인메뉴로
		if(!isAdmin()) {
			return;
		}
		int submenu = -1;
	...
	private static void printCategoty() {
	...
	private static boolean isAdmin() {
		if(user == null || user.getAuthority() != Authority.ADMIN) {
			printStr("관리자가 아닙니다. 해당기능을 이용할 수 없습니다.");
			return false;
		}
		return true;
	}
	...
	private static Member inputMember() {
		...
		// memberList가 0이면 ADMIN권한으로 아니면 MEMBER권한으로 
	...
	private static boolean checkLogin(boolean res) {
		if(user != null && res) {
			printStr("로그인한 사용자은 해당 기능을 이용할 수 없습니다.");
			return true;
