...
CREATE TABLE `mojor` (
	...
	`de_state`	varchar(10)	not NULL default '전공'
);

DROP TABLE IF EXISTS `professe`;

CREATE TABLE `professe` (
...
ALTER TABLE `professe` ADD CONSTRAINT `PK_PROFESSE` PRIMARY KEY (
	`pr_num`
);


ALTER TABLE `student` ADD CONSTRAINT `FK_professe_TO_student_1` FOREIGN KEY (
	`st_pr_num`
)
REFERENCES `professe` (
	`pr_num`
...
ALTER TABLE `department` ADD CONSTRAINT `FK_professe_TO_department_1` FOREIGN KEY (
	`de_pr_num`
)
REFERENCES `professe` (
	`pr_num`
...
ALTER TABLE `professe` ADD CONSTRAINT `FK_department_TO_professe_1` FOREIGN KEY (
	`pr_de_num`
)
REFERENCES `department` (
	`de_num`
...
ALTER TABLE `lecture` ADD CONSTRAINT `FK_professe_TO_lecture_1` FOREIGN KEY (
	`le_pr_num`
)
REFERENCES `professe` (
	`pr_num`
