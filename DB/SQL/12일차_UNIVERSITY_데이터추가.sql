...
-- 135, '전자 공학부', 'KH교육원 1관 3층', '02-123-1234'
INSERT INTO department values(160, '컴퓨터 공학부', 'KH교육원 1관 1층',  '02-123-4567',null),
(123, '기계 공학부', 'KH교육원 1관 2층', '02-123-4568',null),
(135, '전자 공학부', 'KH교육원 1관 3층', '02-123-1234',null);

-- 컴퓨터 공학부에 2022년 홍길동 교수님이 오셨다. 이 때 홍길동의 정보를 추가하는 작업
-- 이 때 교수님 학번은 2022160001이 할당 되어야 함
-- 교수 정보를 추가하기 위한 프로시저 (입학년도, 이름, 학부명, 교수직위, 전화번호)
DROP PROCEDURE IF EXISTS insert_professe;
DELIMITER //
CREATE PROCEDURE insert_professe(
	in _year int,
    in _name varchar(20),
    in _dep varchar(20),
...
BEGIN
    ...
    declare _professe_count int default 0;
    declare _professe_num char(10);
    ...
    if _dep_num is not null then
		set _professe_count =
			(select count(*) + 1 from professe where pr_num like concat(_year,_dep_num,'%'));
		set _professe_num = concat(_year, _dep_num, lpad(_professe_count, 3, '0'));
		insert into professe values(_professe_num, _name, _state, _dep_num, _tel);
    end if;
END //
DELIMITER ;
CALL insert_professe(2022, '홍길동', '컴퓨터 공학부', null, null);
CALL insert_professe(2022, '임꺽정', '컴퓨터 공학부', '정교수', '010-1234-5678');

-- 학생 정보 추가를 위한 프로시저 ( 신입생 추가 )
-- 2022년에 고길동 학생이 컴퓨터 공학부에 입학. 지도 교수님은 같은 과 홍길동 교수님.
...
BEGIN
    ...
    if _dep_num is not null then
        ...
        set _professe_num =
			(select * from professe 
            where substring(pr_num, 5, 3) like _dep_num and pr_name like _professe_name);
        if _professe_num is not null then
			insert into student 
				values(_student_num, _name, 1, '재학', _professe_num);
			insert into mojor(mo_st_num, mo_de_num_) values(_student_num, _dep_num);
		end if;
    end if;
END //
DELIMITER ;
...
-- 2022년 1학기 컴퓨터 개론을 수강하는 고길동 학생의 성적을 등록
-- 중간 100, 기말 80, 출석 100, 과제 100, 비율은 중간 4, 기말4, 출석 1, 과제1로
-- 성적이 계산되어 총점에 저장
...
-- 중간 100, 기말 80, 출석 100, 과제 90, 비율은 중간 4, 기말 4, 출석 1, 과제 1로
-- 성적이 계산되어 총점에 저장
-- 학점은 100이하 90이상 A, 90미만 80이상 B, 80미만 70이상 C, 70미만 60이상 D, 나머지 F
-- 수강번호가 2번
...
BEGIN
    ...
    set new._total = new.sc_mid * 0.4 + new.sc_final * 0.4 
					+ new.sc_attendance * 0.1 + new.sc_homewokre * 0.1;
