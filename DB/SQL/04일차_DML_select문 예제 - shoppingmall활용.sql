...
select 속성 | 식 등
	[from 테이블명]
    [where 조건절] -- 조건에 맞는 결과를 조회 ; ==가 아닌 = , 같지 않은 경우는 !=를 이용
    -- 속성명1로 정렬(오름차순|내림차순)한 후, 속성명1의 값이 같은 행들을 속성명2로 정렬(오름|내림)
    -- 정렬방법이 생략되면 생략되면 asc(오름차순)
...
select * from board where bo_reg_date
	between '2002-01-01 00:00:00' and '2002-12-31 23:59:59';
...
-- 가전 제품(제품카테고리 1번) 중 가장 산 제품보다 가격이 높은 제품들
select * from product 
